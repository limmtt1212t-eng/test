...
as "Номер_договора",
d_sign_contract
as "Дата_подписания_договора",
document_status
as "Статус_документа",
object_count
as "Объектов_в_договоре",
object_number
as "Номер_объекта_в_договоре",
object_id
as "ID_объекта",
object_description
as "Описание_объекта",
...
as "Площадь_из_характеристик",
characteristics ->> 'construction_year'
as "Год_постройки",
characteristics ->> 'total_floors_count'
as "Этажность",
characteristics ->> 'load_bearing_walls_material'
as "Материал_несущих_стен",
insurance_value
as "Страховая_стоимость",
insurance_value_currency
as "Валюта_страховой_стоимости",
insurance_value_basis
as "Основание_страховой_стоимости",
is_pledged
as "Есть_залог",
pledged_value
as "Залоговая_стоимость",
object_insured_sum
as "Страховая_сумма_объекта",
object_insured_sum_currency
as "Валюта_страховой_суммы",
crm_industry
as "Отрасль_из_CRM",
task_industry
as "Отрасль_из_задачи",
task_subindustry
as "Подотрасль_из_задачи",
crm_okved
as "ОКВЭД",
crm_segment
as "Сегмент_из_CRM",
...
as "JSON_характеристик_заполнен",
case
when nullif(
regexp_replace(
...
as "Есть_ИНН_для_связи_со_СПАРК",
case
when nullif(btrim(fias_code), '') is not null
then 'Да'
else 'Нет'
end
as "Есть_ФИАС_для_поиска_в_ЕГРН"
from display_rows
order by
d_sign_contract desc nulls last,
contract_id,
