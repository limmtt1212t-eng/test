/*
МОДЕЛЬ 1. ОБЪЕКТНАЯ ВЫБОРКА ИЗ «СФЕРЫ»
======================================

Назначение:
собрать основу данных для модели оценки стоимости недвижимости ЮЛ.

Единица результата:
одна строка = один object_id в одной выбранной задаче конкретного договора.

Если один и тот же физический объект встречается в разных договорах или
периодах, он намеренно может появиться несколько раз: это разные моменты
страхования.

Что делает запрос:
1. Использует переданное правило отбора задач оформления.
2. Выбирает одну наиболее позднюю подходящую задачу на договор.
3. Берёт только недвижимость ЮЛ, без BI.
4. Убирает повторные связи одного object_id внутри выбранной задачи.
5. Сворачивает несколько вариантов условий страхования в одну строку.
6. Сохраняет ключи для последующего обогащения:
   - normalized_policyholder_inn — СПАРК;
   - fias_code, адрес и координаты — ЕГРН/геоисточники;
   - insurance_object_id — будущая таблица соответствий объекта с ЕГРН.

Ограничения:
- operational_archive — статус задачи, а не доказательство юридически
  заключённого договора;
- правило выбора задачи пока техническое и требует подтверждения владельца
  данных;
- insurance_value, pledged_value и insured_sum выводятся как доступные
  значения, но ни одно из них пока не назначено целевой переменной модели;
- внешние источники здесь не присоединяются: они находятся в других БД
  или API и должны сначала быть сохранены отдельными снимками;
- общие сумма и премия договора повторяются на всех объектах договора,
  поэтому их нельзя суммировать по строкам результата.

Безопасный первый запуск:
в CTE sampled_tasks оставлен LIMIT 200 договоров. После проверки результата
его можно удалить для полной выборки.
...
sampled_tasks as (
    /*
    LIMIT применяется к договорам, а не к объектным строкам.
    Для полной выборки удалить только строку "limit 200".
    */
    select *
...
object_link_candidates as (
    select
        ...
        /*
        Первые физические признаки извлекаются текстом без преобразования
        в число. Это защищает запрос от ошибки, если в JSON встретится
        нестандартное значение. Нормализация типов выполняется позднее.
        */
...
condition_summary as (
    /*
    У одной версии характеристик может быть несколько вариантов условий.
    Прямой JOIN размножил бы объектные строки, поэтому условия сворачиваются.
    */
    select
...
object_occurrence_summary as (
    /*
    Считает, в скольких выбранных договорах встречается object_id.
    Это проверка только текущего среза из sampled_tasks.
    */
    select
        insurance_object_id,
...
select
    ...
    /*
    Поля уровня договора/задачи повторяются для каждого объекта договора.
    */
